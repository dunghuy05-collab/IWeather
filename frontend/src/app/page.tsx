...
export default function Home() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (interests.length === 0) {
      setSubmitState({ status: "error", message: "Hãy chọn ít nhất một sở thích." });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error?.detail?.[0]?.msg ?? "Không thể lưu yêu cầu chuyến đi.");
      }
  ...
  async function handlePlan() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result?.detail ?? "Không thể tạo lịch trình AI.");
      }
      setPlanState({ status: "success", content: result.content });
    } catch (error) {
      setPlanState({
        status: "error",
        message: error instanceof Error ? error.message : "Không thể tạo lịch trình AI.",
      });
    }
  }
      ...
      <div className="mx-auto max-w-7xl px-5 py-6 sm:px-10 lg:px-14">
        <nav className="flex items-center justify-between border-b border-[#17352d]/15 pb-5">
          <div className="flex items-center gap-3">
            <span className="grid size-10 place-items-center rounded-full bg-[#17352d] text-lg text-white">✦</span>
            <div>
        ...
        <section className="grid gap-12 py-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-start lg:py-16">
          <div className="lg:sticky lg:top-10">
            <p className="mb-5 text-sm font-semibold uppercase tracking-[0.25em] text-[#c55d32]">Bắt đầu hành trình</p>
            <h1 className="text-5xl font-semibold leading-[1.03] tracking-[-0.05em] sm:text-6xl">
              Chuyến đi của bạn,
              <span className="block font-serif italic text-[#c55d32]">theo đúng cách bạn muốn.</span>
            </h1>
            <p className="mt-7 max-w-lg text-lg leading-8 text-[#557168]">
              Cho chúng tôi biết những điều quan trọng. AI sẽ dùng thông tin này để tạo lịch trình phù hợp với ngân sách và sở thích của bạn.
            </p>
            <div className="mt-9 grid grid-cols-3 gap-3 text-center">
              {[["01", "Nhu cầu"], ["02", "Phân tích"], ["03", "Lịch trình"]].map(([step, label]) => (
          ...
          <form className="rounded-[2rem] border border-white bg-white/85 p-6 shadow-[0_28px_80px_-45px_rgba(23,53,45,0.5)] sm:p-9" onSubmit={handleSubmit}>
            <div className="mb-8">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#789087]">Thông tin chuyến đi</p>
              <h2 className="mt-2 text-3xl font-semibold tracking-tight">Bạn muốn đi đâu?</h2>
            </div>

            <div className="grid gap-6 sm:grid-cols-2">
              <label className="sm:col-span-2">
                <span className="field-label">Điểm đến</span>
                <input className="field-input" name="destination" placeholder="Ví dụ: Đà Nẵng, Việt Nam" minLength={2} maxLength={120} required />
              </label>

              <label>
                <span className="field-label">Ngân sách</span>
                <div className="flex gap-2">
                  ...
                  <select className="field-input w-24" name="currency" defaultValue="USD">
                    <option>USD</option><option>VND</option><option>EUR</option><option>THB</option>
                  </select>
                </div>
              </label>
              ...
              <label className="sm:col-span-2">
                <span className="field-label">Phong cách du lịch</span>
                <select className="field-input" name="travel_style" defaultValue="comfort" required>
                  <option value="backpacking">Tiết kiệm / Backpacking</option>
                  <option value="comfort">Thoải mái</option>
                  <option value="luxury">Cao cấp</option>
                  <option value="family">Gia đình</option>
                  <option value="adventure">Phiêu lưu</option>
                  <option value="culture">Khám phá văn hóa</option>
                </select>
              </label>
            </div>

            <fieldset className="mt-7">
              <legend className="field-label">Bạn quan tâm điều gì?</legend>
                ...
                {interestOptions.map((interest) => {
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-full border px-4 py-2.5 text-sm font-medium transition ${selected ? "border-[#17352d] bg-[#17352d] text-white" : "border-[#17352d]/15 bg-[#f8faf6] text-[#49665c] hover:border-[#17352d]/40"}`}
                      key={interest}
            ...
            <label className="mt-7 block">
              <span className="field-label">Ghi chú thêm <span className="font-normal text-[#8da097]">(không bắt buộc)</span></span>
              <textarea className="field-input min-h-28 resize-y" name="notes" maxLength={1000} placeholder="Ví dụ: Đi cùng trẻ nhỏ, ưu tiên lịch trình thư thả..." />
            </label>
            ...
            {submitState.status === "success" && (
              <div className="mt-5 rounded-xl bg-[#e5f2e7] px-4 py-3 text-sm text-[#28573c]">
                <p>
                  Đã lưu yêu cầu thành công. Mã chuyến đi: <span className="font-mono font-semibold">{submitState.id}</span>
                </p>
                <button
                  ...
                  type="button"
                >
                  {planState.status === "submitting" ? "Đang tạo lịch trình..." : "Tạo lịch trình bằng AI"}
                </button>
              </div>
            )}
            ...
            {planState.status === "success" && (
              <article className="mt-5 rounded-xl border border-[#17352d]/10 bg-[#f8faf6] px-5 py-4 text-sm leading-7 text-[#315b48]">
                <h3 className="font-semibold text-[#17352d]">Lịch trình đề xuất</h3>
            ...
            <button className="mt-7 w-full rounded-2xl bg-[#c55d32] px-6 py-4 font-semibold text-white shadow-lg shadow-[#c55d32]/20 transition hover:bg-[#af4d29] disabled:cursor-wait disabled:opacity-60" disabled={submitState.status === "submitting"} type="submit">
              {submitState.status === "submitting" ? "Đang lưu yêu cầu..." : "Tạo yêu cầu chuyến đi →"}
            </button>
          </form>
        </section>
      </div>
